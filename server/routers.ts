...
export const appRouter = router({
  ...
  engine: router({
    ...
    updateTill: protectedProcedure.input(z.object({ id: z.number().int().positive(), tillNumber: z.string().regex(/^\d{5,8}$/), name: z.string().min(2).max(80), location: z.string().max(120).optional(), isActive: z.boolean(), payoutPhone: z.string().regex(/^(?:254|0)(?:7|1)\d{8}$/, "Enter a valid Kenyan payout phone number").transform((value) => value.startsWith("0") ? `254${value.slice(1)}` : value) })).mutation(({ ctx, input }) => updateTill(ctx.user.id, input.id, input)),
