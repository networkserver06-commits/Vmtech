...
async function ensureApiKeyRecoveryColumn() { try { await execute("ALTER TABLE apiKeys ADD COLUMN keyEncrypted text"); } catch { /* Column already exists or deployment user cannot alter schema. */ } }
export async function insertApiKey(input: { userId: number; name: string; keyHash: string; keyEncrypted: string }) { try { return await execute({ sql: "INSERT INTO apiKeys (userId, name, keyHash, keyPrefix, keyEncrypted) VALUES (?, ?, ?, 'sk_live_', ?)", args: [input.userId, input.name, input.keyHash, input.keyEncrypted] }); } catch { await ensureApiKeyRecoveryColumn(); try { return await execute({ sql: "INSERT INTO apiKeys (userId, name, keyHash, keyPrefix, keyEncrypted) VALUES (?, ?, ?, 'sk_live_', ?)", args: [input.userId, input.name, input.keyHash, input.keyEncrypted] }); } catch { return execute({ sql: "INSERT INTO apiKeys (userId, name, keyHash, keyPrefix) VALUES (?, ?, ?, 'sk_live_')", args: [input.userId, input.name, input.keyHash] }); } } }
export async function listApiKeys(userId: number) { let result; try { result = await execute({ sql: "SELECT id, name, keyPrefix, keyEncrypted, isActive, lastUsedAt, createdAt FROM apiKeys WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); } catch { await ensureApiKeyRecoveryColumn(); try { result = await execute({ sql: "SELECT id, name, keyPrefix, keyEncrypted, isActive, lastUsedAt, createdAt FROM apiKeys WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); } catch { result = await execute({ sql: "SELECT id, name, keyPrefix, isActive, lastUsedAt, createdAt FROM apiKeys WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); } } return result ? asRows<TursoRow>(result).map((row) => { const encrypted = String(row.keyEncrypted ?? ""); let key: string | null = null; if (encrypted) { try { key = decryptSecret(encrypted); } catch { key = null; } } const { keyEncrypted: _keyEncrypted, ...safe } = row; return { ...safe, key, recoverable: Boolean(key) }; }) : []; }
...
export async function createTill(input: { userId: number; tillNumber: string; name: string; location?: string | null; paymentType: "BUY_GOODS" | "PAYBILL" | "PHONE"; businessShortcode?: string | null; payoutPhone: string }) { return execute({ sql: "INSERT INTO tills (userId, tillNumber, name, location, paymentType, businessShortcode, payoutPhone) VALUES (?, ?, ?, ?, ?, ?, ?)", args: [input.userId, input.tillNumber, input.name, input.location ?? null, input.paymentType, input.businessShortcode ?? null, input.payoutPhone] }); }
