...
const SIGNUP_BONUS_REFERENCE = (userId: number) => `SIGNUP_BONUS_${userId}`;
...
export async function getOverviewData(userId: number) {
  await reconcileWalletDeposits(userId);
  ...
  return { balance: Number(asRows<TursoRow>(wallet)[0]?.balance ?? 0), collections: collectionRows.filter((row) => String(row.status).toUpperCase() === "SUCCESS").reduce((sum, row) => sum + Number(row.amount), 0), payouts: 0, successRate: total ? Math.round((successful / total) * 1000) / 10 : 0, activeKeys: keys.rows.length, accountId: user?.accountId ?? "—", environment: config?.environment === "PRODUCTION" || (!config && envLive) ? "PRODUCTION" : config?.environment === "SANDBOX" || !envLive ? "SANDBOX" : "NOT_CONFIGURED", shortcode: config?.shortcode ? String(config.shortcode) : process.env.MPESA_SHORTCODE ?? "4208798", transactions: activity };
}

export async function insertApiKey(input: { userId: number; name: string; keyHash: string; keyEncrypted: string }) { return execute({ sql: "INSERT INTO apiKeys (userId, name, keyHash, keyEncrypted, keyPrefix) VALUES (?, ?, ?, ?, 'sk_live_')", args: [input.userId, input.name, input.keyHash, input.keyEncrypted] }); }
export async function listApiKeys(userId: number) { const result = await execute({ sql: "SELECT id, name, keyPrefix, keyEncrypted, isActive, lastUsedAt, createdAt FROM apiKeys WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result).map((row) => { let key: string | null = null; if (typeof row.keyEncrypted === "string" && row.keyEncrypted) { try { key = decryptSecret(row.keyEncrypted); } catch (error) { console.error("API key decryption failed", error); } } return { ...row, key, recoverable: Boolean(key) }; }) : []; }
...
export async function deleteApiKey(userId: number, id: number) { return execute({ sql: "DELETE FROM apiKeys WHERE id = ? AND userId = ?", args: [id, userId] }); }
export async function listTills(userId: number) { const result = await execute({ sql: "SELECT id, tillNumber, name, location, paymentType, businessShortcode, payoutPhone, isActive, createdAt FROM tills WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result) : []; }
export async function createTill(input: { userId: number; tillNumber: string; name: string; location?: string | null; paymentType: "BUY_GOODS" | "PAYBILL" | "PHONE"; businessShortcode?: string | null; payoutPhone: string }) { return execute({ sql: "INSERT INTO tills (userId, tillNumber, name, location, paymentType, businessShortcode, payoutPhone) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(userId, tillNumber) DO UPDATE SET name = excluded.name, location = excluded.location, paymentType = excluded.paymentType, businessShortcode = excluded.businessShortcode, payoutPhone = excluded.payoutPhone, isActive = 1", args: [input.userId, input.tillNumber, input.name, input.location ?? null, input.paymentType, input.businessShortcode ?? null, input.payoutPhone] }); }
export async function updateTill(userId: number, id: number, input: { tillNumber: string; name: string; location?: string | null; isActive: boolean; payoutPhone: string }) { return execute({ sql: "UPDATE tills SET tillNumber = ?, name = ?, location = ?, payoutPhone = ?, isActive = ? WHERE id = ? AND userId = ?", args: [input.tillNumber, input.name, input.location ?? null, input.payoutPhone, input.isActive ? 1 : 0, id, userId] }); }
...
async function reconcileSuccessfulStkFees(userId: number) {
  ...
  for (const row of rows) {
    ...
    if (Number(charged.rowsAffected ?? 0) !== 1) { await db.execute({ sql: "UPDATE transactions SET platformFee = '0.00', netAmount = NULL, feeChargedAt = NULL WHERE id = ? AND feeChargedAt IS NOT NULL", args: [Number(row.id)] }); continue; }
    await db.execute({ sql: "INSERT OR IGNORE INTO walletTransactions (walletId, amount, type, reference, description) SELECT id, ?, 'PLATFORM_FEE', ?, ? FROM wallets WHERE userId = ?", args: [(-fee).toFixed(2), reference, `Platform fee for STK Push transaction ${row.id}`, userId] });
  }
}
...
export async function listTransactionHistory(userId: number) {
  const [collections, payouts, deposits] = await Promise.all([
    ...
    execute({ sql: "SELECT * FROM payouts WHERE userId = ?", args: [userId] }),
    execute({ sql: "SELECT * FROM walletDeposits WHERE userId = ?", args: [userId] }),
  ]);
  const history: Array<TursoRow & { kind: string }> = [...(collections ? asRows<TursoRow>(collections) : []).map((row) => ({ ...row, kind: "COLLECTION" })), ...(payouts ? asRows<TursoRow>(payouts) : []).map((row) => ({ ...row, kind: "PAYOUT" })), ...(deposits ? asRows<TursoRow>(deposits) : []).map((row) => ({ ...row, kind: "WALLET_DEPOSIT" }))];
  return history.sort((a, b) => new Date(String(b.createdAt)).getTime() - new Date(String(a.createdAt)).getTime());
}
export async function getTransactionStatus(userId: number, checkoutRequestId: string) {
  const db = await getTurso(); if (!db) return undefined;
  const row = asRows<TursoRow>(await db.execute({ sql: "SELECT checkoutRequestId, amount, status, failureReason, mpesaReceipt, createdAt FROM transactions WHERE userId = ? AND checkoutRequestId = ? LIMIT 1", args: [userId, checkoutRequestId] }))[0];
  return row;
}
...
export async function insertTransaction(input: { userId: number; checkoutRequestId: string; merchantRequestId?: string; accountReference: string; phoneNumber: string; amount: number; tillId?: number | null; status?: string; failureReason?: string | null; feeReservationReference?: string | null }) { return execute({ sql: "INSERT INTO transactions (userId, checkoutRequestId, merchantRequestId, tillId, accountReference, phoneNumber, amount, status, netAmount, failureReason, feeReservedAt, feeReservationReference) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", args: [input.userId, input.checkoutRequestId, input.merchantRequestId ?? null, input.tillId ?? null, input.accountReference, input.phoneNumber, input.amount.toFixed(2), input.status ?? "PENDING", input.amount.toFixed(2), input.failureReason ?? null, input.feeReservationReference ? now() : null, input.feeReservationReference ?? null] }); }
...
export async function createPayoutRequest(input: { userId: number; transactionId: number; amount: number; destinationType: "PHONE" | "TILL"; destination: string }) {
  ...
  const collectionAmount = Number(transaction.amount ?? 0);
  if (!Number.isFinite(collectionAmount) || collectionAmount < 50) throw new Error("Only successful collections of at least KES 50 can be requested for payout");
  if (!Number.isFinite(input.amount) || input.amount < 50 || input.amount > collectionAmount) throw new Error(`Payout amount must be between KES 50 and ${collectionAmount.toFixed(2)}`);
  const result = await db.execute({ sql: "INSERT INTO payoutRequests (userId, transactionId, amount, destinationType, destination) SELECT ?, ?, ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM payoutRequests WHERE transactionId = ?)", args: [input.userId, input.transactionId, input.amount.toFixed(2), input.destinationType, input.destination, input.transactionId] });
  if (Number(result.rowsAffected ?? 0) !== 1) throw new Error("This collection already has a payout request");
  return { id: Number(result.lastInsertRowid ?? 0), status: "REQUESTED", amount: input.amount };
}
...
export async function reconcileWalletDeposits(userId?: number) {
  const db = await getTurso(); if (!db) return;
  const rows = asRows<TursoRow>(await db.execute({ sql: "SELECT wd.id, wd.userId, wd.checkoutRequestId, wd.amount FROM walletDeposits wd WHERE wd.status = 'SUCCESS' AND (? IS NULL OR wd.userId = ?) AND NOT EXISTS (SELECT 1 FROM walletTransactions wt JOIN wallets w ON w.id = wt.walletId WHERE w.userId = wd.userId AND wt.reference = ('WALLET_DEPOSIT_' || CAST(wd.id AS TEXT))) ORDER BY wd.id ASC LIMIT 200", args: [userId ?? null, userId ?? null] }));
  for (const row of rows) {
    const amount = Number(row.amount ?? 0); const reference = `WALLET_DEPOSIT_${String(row.id)}`;
    await db.batch([{ sql: "INSERT OR IGNORE INTO wallets (userId, balance) VALUES (?, '0.00')", args: [Number(row.userId)] }, { sql: "UPDATE wallets SET balance = CAST(balance AS REAL) + ?, updatedAt = ? WHERE userId = ? AND NOT EXISTS (SELECT 1 FROM walletTransactions wt JOIN wallets w ON w.id = wt.walletId WHERE w.userId = ? AND wt.reference = ?)", args: [amount.toFixed(2), now(), Number(row.userId), Number(row.userId), reference] }, { sql: "INSERT OR IGNORE INTO walletTransactions (walletId, amount, type, reference, description) SELECT id, ?, 'DEPOSIT', ?, ? FROM wallets WHERE userId = ?", args: [amount.toFixed(2), reference, `Wallet deposit via STK Push ${String(row.checkoutRequestId)}`, Number(row.userId)] }], "write");
  }
}
export async function listWalletDeposits(userId: number) { await reconcileWalletDeposits(userId); const result = await execute({ sql: "SELECT id, checkoutRequestId, phoneNumber, amount, status, mpesaReceipt, failureReason, createdAt, settledAt FROM walletDeposits WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result) : []; }
export async function listUserWalletLedger(userId: number) { await reconcileSuccessfulStkFees(userId); const result = await execute({ sql: "SELECT wt.id, wt.amount, wt.type, wt.reference, wt.description, wt.createdAt, t.id AS transactionId, t.checkoutRequestId, t.accountReference, t.amount AS collectionAmount, t.platformFee, t.netAmount, t.mpesaReceipt FROM walletTransactions wt JOIN wallets w ON w.id = wt.walletId LEFT JOIN transactions t ON wt.reference = ('STK_FEE_' || CAST(t.id AS TEXT)) WHERE w.userId = ? ORDER BY datetime(wt.createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result) : []; }
...
export async function reserveWalletFee(input: { userId: number; amount: number; reference: string }) {
  ...
  await db.execute({ sql: "INSERT OR IGNORE INTO wallets (userId, balance) VALUES (?, '0.00')", args: [input.userId] });
  const result = await db.execute({ sql: "UPDATE wallets SET balance = CAST(balance AS REAL) - ?, updatedAt = ? WHERE userId = ? AND CAST(balance AS REAL) >= ?", args: [fee.toFixed(2), timestamp, input.userId, fee.toFixed(2)] });
  if (Number(result.rowsAffected ?? 0) !== 1) return null;
  try {
    await db.execute({ sql: "INSERT INTO walletTransactions (walletId, amount, type, reference, description) SELECT id, ?, 'PLATFORM_FEE_RESERVED', ?, ? FROM wallets WHERE userId = ?", args: [(-fee).toFixed(2), input.reference, `Reserved platform fee for STK Push (${input.amount.toFixed(2)} KES)`, input.userId] });
  } catch (error) {
    await db.execute({ sql: "UPDATE wallets SET balance = CAST(balance AS REAL) + ?, updatedAt = ? WHERE userId = ?", args: [fee.toFixed(2), now(), input.userId] });
    throw error;
  }
  return { fee, reference: input.reference };
}
export async function releaseWalletFee(input: { userId: number; fee: number; reference: string; reason: string }) {
  const db = await getTurso(); if (!db) return null;
...
async function releaseTransactionFee(checkoutRequestId: string, reason: string) {
  const db = await getTurso(); if (!db) return null;
  const row = asRows<TursoRow>(await db.execute({ sql: "SELECT userId, amount, feeReservationReference, feeReleasedAt FROM transactions WHERE checkoutRequestId = ? LIMIT 1", args: [checkoutRequestId] }))[0];
  if (!row?.feeReservationReference || row.feeReleasedAt) return null;
  return releaseWalletFee({ userId: Number(row.userId), fee: calculatePlatformFee(Number(row.amount)), reference: String(row.feeReservationReference), reason });
}
export async function updateStkCallback(input: { checkoutRequestId: string; success: boolean; status?: "SUCCESS" | "FAILED" | "CANCELLED"; failureReason?: string | null; receipt?: string | null; paidAmount?: number | null; paidPhoneNumber?: string | null }) {
  ...
  if (deposit) {
    ...
    const result = input.success
      ? (await db.batch([{ sql: "INSERT OR IGNORE INTO wallets (userId, balance) VALUES (?, '0.00')", args: [Number(deposit.userId)] }, { sql: "UPDATE walletDeposits SET status = 'SUCCESS', failureReason = NULL, mpesaReceipt = ?, settledAt = ? WHERE checkoutRequestId = ? AND status = 'PENDING'", args: [input.receipt ?? null, now(), input.checkoutRequestId] }, { sql: "UPDATE wallets SET balance = CAST(balance AS REAL) + ?, updatedAt = ? WHERE userId = ? AND NOT EXISTS (SELECT 1 FROM walletTransactions WHERE reference = ?)", args: [amount.toFixed(2), now(), Number(deposit.userId), reference] }, { sql: "INSERT OR IGNORE INTO walletTransactions (walletId, amount, type, reference, description) SELECT id, ?, 'DEPOSIT', ?, ? FROM wallets WHERE userId = ?", args: [amount.toFixed(2), reference, `Wallet deposit via STK Push ${input.checkoutRequestId}`, Number(deposit.userId)] }], "write"))[1]
      : await db.execute({ sql: "UPDATE walletDeposits SET status = 'FAILED', failureReason = ?, mpesaReceipt = ?, settledAt = NULL WHERE checkoutRequestId = ? AND status = 'PENDING'", args: [input.failureReason ?? null, input.receipt ?? null, input.checkoutRequestId] });
    if (Number(result.rowsAffected ?? 0) === 1) void dispatchUserWebhooks(Number(deposit.userId), `wallet.deposit.${input.success ? "success" : "failed"}`, { depositId: Number(deposit.id), checkoutRequestId: input.checkoutRequestId, phoneNumber: String(deposit.phoneNumber), amount, status: input.success ? "SUCCESS" : "FAILED", failureReason: input.failureReason ?? null, mpesaReceipt: input.receipt ?? null }).catch((error) => console.error("wallet webhook dispatch failed", error));
    return result;
  }
  if (input.success && !input.receipt) { const failed = await db.execute({ sql: "UPDATE transactions SET status = 'FAILED', failureReason = ? WHERE checkoutRequestId = ? AND status = 'PENDING'", args: ["Safaricom callback did not include a payment receipt", input.checkoutRequestId] }); if (Number(failed.rowsAffected ?? 0) === 1) await releaseTransactionFee(input.checkoutRequestId, "Released platform fee because the payment callback had no receipt."); return failed; }
  ...
  if (input.success && transaction && (input.paidAmount == null || Math.abs(Number(transaction.amount) - input.paidAmount) > 0.001 || (input.paidPhoneNumber && String(transaction.phoneNumber) !== input.paidPhoneNumber))) { const failed = await db.execute({ sql: "UPDATE transactions SET status = 'FAILED', failureReason = ? WHERE checkoutRequestId = ? AND status = 'PENDING'", args: ["Safaricom callback payment details did not match the pending transaction", input.checkoutRequestId] }); if (Number(failed.rowsAffected ?? 0) === 1) await releaseTransactionFee(input.checkoutRequestId, "Released platform fee because the payment callback details did not match."); return failed; }
  const finalStatus = input.status ?? (input.success ? "SUCCESS" : "FAILED");
  const result = await db.execute({ sql: "UPDATE transactions SET status = ?, failureReason = ?, mpesaReceipt = ? WHERE checkoutRequestId = ? AND status NOT IN ('SUCCESS', 'FAILED', 'CANCELLED')", args: [finalStatus, input.failureReason ?? null, input.receipt ?? null, input.checkoutRequestId] });
  const row = asRows<TursoRow>(await db.execute({ sql: "SELECT id, userId, amount, accountReference, phoneNumber, status, failureReason, mpesaReceipt, createdAt, feeReservationReference FROM transactions WHERE checkoutRequestId = ? LIMIT 1", args: [input.checkoutRequestId] }))[0];
  if (row && Number(result.rowsAffected ?? 0) === 1) void dispatchUserWebhooks(Number(row.userId), `payment.${String(row.status).toLowerCase()}`, { transactionId: Number(row.id), checkoutRequestId: input.checkoutRequestId, accountReference: String(row.accountReference), phoneNumber: String(row.phoneNumber), amount: Number(row.amount), status: String(row.status), failureReason: row.failureReason == null ? null : String(row.failureReason), mpesaReceipt: row.mpesaReceipt == null ? null : String(row.mpesaReceipt), createdAt: String(row.createdAt) }).catch((error) => console.error("STK webhook dispatch failed", error));
  if (finalStatus !== "SUCCESS" || Number(result.rowsAffected ?? 0) !== 1) { if (Number(result.rowsAffected ?? 0) === 1) await releaseTransactionFee(input.checkoutRequestId, "Released platform fee because the payment did not complete successfully."); return result; }
  if (!row) return result;
  const amount = Number(row.amount ?? 0); const fee = calculatePlatformFee(amount); const net = Math.max(0, amount - fee); const reference = String(row.feeReservationReference ?? `STK_FEE_${String(row.id)}`);
  if (row.feeReservationReference) {
    await db.batch([{ sql: "UPDATE transactions SET platformFee = ?, netAmount = ?, feeChargedAt = ? WHERE id = ? AND feeChargedAt IS NULL", args: [fee.toFixed(2), net.toFixed(2), now(), Number(row.id)] }, { sql: "UPDATE walletTransactions SET type = 'PLATFORM_FEE', description = ? WHERE reference = ?", args: [`Platform fee for STK Push ${input.checkoutRequestId}`, reference] }], "write");
  } else {
    const charged = await db.execute({ sql: "UPDATE wallets SET balance = CAST(balance AS REAL) - ?, updatedAt = ? WHERE userId = ? AND CAST(balance AS REAL) >= ?", args: [fee.toFixed(2), now(), Number(row.userId), fee.toFixed(2)] });
    if (Number(charged.rowsAffected ?? 0) === 1) await db.batch([{ sql: "UPDATE transactions SET platformFee = ?, netAmount = ?, feeChargedAt = ? WHERE id = ? AND feeChargedAt IS NULL", args: [fee.toFixed(2), net.toFixed(2), now(), Number(row.id)] }, { sql: "INSERT OR IGNORE INTO walletTransactions (walletId, amount, type, reference, description) SELECT id, ?, 'PLATFORM_FEE', ?, ? FROM wallets WHERE userId = ?", args: [(-fee).toFixed(2), reference, `Platform fee for STK Push ${input.checkoutRequestId}`, Number(row.userId)] }], "write");
  }
  return result;
}
