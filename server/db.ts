...
export async function createTill(input: { userId: number; tillNumber: string; name: string; location?: string | null; paymentType: "BUY_GOODS" | "PAYBILL"; businessShortcode?: string | null; payoutPhone: string }) { return execute({ sql: "INSERT INTO tills (userId, tillNumber, name, location, paymentType, businessShortcode, payoutPhone) VALUES (?, ?, ?, ?, ?, ?, ?)", args: [input.userId, input.tillNumber, input.name, input.location ?? null, input.paymentType, input.businessShortcode ?? null, input.payoutPhone] }); }
