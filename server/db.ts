...
export async function insertApiKey(input: { userId: number; name: string; keyHash: string; keyEncrypted: string }) { return execute({ sql: "INSERT INTO apiKeys (userId, name, keyHash, keyPrefix, keyEncrypted) VALUES (?, ?, ?, 'sk_live_', ?)", args: [input.userId, input.name, input.keyHash, input.keyEncrypted] }); }
