...
export async function listApiKeys(userId: number) { const result = await execute({ sql: "SELECT id, name, keyPrefix, isActive, lastUsedAt, createdAt FROM apiKeys WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result).map((row) => ({ ...row, recoverable: false })) : []; }
