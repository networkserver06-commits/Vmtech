...
export async function listTills(userId: number) { const result = await execute({ sql: "SELECT id, tillNumber, name, location, isActive, createdAt FROM tills WHERE userId = ? ORDER BY datetime(createdAt) DESC", args: [userId] }); return result ? asRows<TursoRow>(result) : []; }
export async function createTill(input: { userId: number; tillNumber: string; name: string; location?: string | null }) { return execute({ sql: "INSERT INTO tills (userId, tillNumber, name, location) VALUES (?, ?, ?, ?)", args: [input.userId, input.tillNumber, input.name, input.location ?? null] }); }
...
export async function getTill(userId: number, id: number) { const result = await execute({ sql: "SELECT id, tillNumber, name, location, isActive FROM tills WHERE id = ? AND userId = ? LIMIT 1", args: [id, userId] }); return result ? asRows<TursoRow>(result)[0] : undefined; }
