...
export async function getDarajaToken(config: DarajaConfig) {
  ...
  const response = await fetch(`${baseUrl}/oauth/v1/generate?grant_type=client_credentials`, { headers: { Authorization: `Basic ${basic}` } });
...
export async function triggerStkPush(config: DarajaConfig, input: { phoneNumber: string; amount: number; accountReference: string; transactionDesc: string; callbackUrl: string }) {
  if (process.env.MPESA_LIVE_ENABLED !== "true") {
    return { sandbox: true, CheckoutRequestID: `ws_CO_${Date.now()}`, MerchantRequestID: "sandbox-merchant", ResponseDescription: "Sandbox mode — request accepted" };
  }
...
export async function triggerB2cPayout(config: DarajaConfig, input: { phoneNumber: string; amount: number; commandId: "BusinessPayment" | "SalaryPayment"; queueTimeoutUrl: string; resultUrl: string }) {
  if (process.env.MPESA_LIVE_ENABLED !== "true") {
    return { sandbox: true, OriginatorConversationID: `sandbox-${Date.now()}`, ConversationID: `sandbox-${Date.now()}`, ResponseDescription: "Sandbox mode — payout queued" };
  }
