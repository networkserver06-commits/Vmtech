...
export default function PaymentLinkCheckout() {
  ...
  const merchantSlug = window.location.pathname.replace(/^\//, "").trim().toLowerCase();
  ...
  const submit = async () => {
    ...
    setPending(true);
    try {
      const merchantQuery = merchantSlug && merchantSlug !== "pay" ? `slug=${encodeURIComponent(merchantSlug)}` : `accountId=${encodeURIComponent(accountId)}`;
