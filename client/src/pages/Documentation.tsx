...
export default function Documentation() {
  ...
  -d '{"phoneNumber":"254712345678","amount":10,"accountReference":"1ORDER001","transactionDesc":"Customer payment"}'`}</code><CopyValue value={`curl -X POST https://leetec.online/api/v1/stkpush -H "Authorization: Bearer sk_live_your_secret_key" -H "Content-Type: application/json" -d '{"phoneNumber":"254712345678","amount":10,"accountReference":"1ORDER001","transactionDesc":"Customer payment"}'`} /></div><p className="settings-note"><ShieldCheck size={15} /> Keep the key in your server environment, such as <code>LEETEC_API_KEY</code>. LeeTec then handles the Daraja credentials securely.</p></section>
        <section className="admin-panel docs-panel"><div className="admin-panel-heading"><div><h2>Node.js integration</h2><p>Use Node.js 18+ and keep the API key in a server environment variable.</p></div><KeyRound size={22} /></div><div className="code-block docs-code-block"><code>{`const response = await fetch("https://leetec.online/api/v1/stkpush", {\n  method: "POST",\n  headers: {\n    Authorization: \`Bearer \${process.env.LEETEC_API_KEY}\`,\n    "Content-Type": "application/json"\n  },\n  body: JSON.stringify({\n    phoneNumber: "254712345678",\n    amount: 10,\n    accountReference: "1ORDER001",\n    transactionDesc: "Customer payment"\n  })\n});\n\nconst result = await response.json();\nif (!response.ok) throw new Error(result.error ?? "STK Push failed");\nconsole.log(result.CheckoutRequestID);`}</code><CopyValue value={`const response = await fetch("https://leetec.online/api/v1/stkpush", { method: "POST", headers: { Authorization: \`Bearer \${process.env.LEETEC_API_KEY}\`, "Content-Type": "application/json" }, body: JSON.stringify({ phoneNumber: "254712345678", amount: 10, accountReference: "1ORDER001", transactionDesc: "Customer payment" }) });`} /></div></section>
        <section className="admin-panel docs-panel"><div className="admin-panel-heading"><div><h2>PHP integration</h2><p>Use cURL from your server and read the JSON response before confirming the order.</p></div><KeyRound size={22} /></div><div className="code-block docs-code-block"><code>{`$ch = curl_init("https://leetec.online/api/v1/stkpush");\ncurl_setopt_array($ch, [\n  CURLOPT_POST => true,\n  CURLOPT_HTTPHEADER => [\n    "Authorization: Bearer " . getenv("LEETEC_API_KEY"),\n    "Content-Type: application/json"\n  ],\n  CURLOPT_POSTFIELDS => json_encode([\n    "phoneNumber" => "254712345678",\n    "amount" => 10,\n    "accountReference" => "1ORDER001",\n    "transactionDesc" => "Customer payment"\n  ]),\n  CURLOPT_RETURNTRANSFER => true\n]);\n$body = curl_exec($ch);\n$status = curl_getinfo($ch, CURLINFO_HTTP_CODE);\ncurl_close($ch);\n$result = json_decode($body, true);\nif ($status < 200 || $status >= 300) {\n  throw new RuntimeException($result["error"] ?? "STK Push failed");\n}`}</code><CopyValue value={`curl_init("https://leetec.online/api/v1/stkpush");`} /></div></section>
        <section className="settings-card docs-panel"><div className="settings-card-heading"><div className="settings-icon"><BookOpen size={16} /></div><div><h2>Request and response</h2><p>Use these fields when creating a payment request.</p></div></div><div className="doc-detail-list"><div><span>phoneNumber</span><strong>2547XXXXXXXX</strong></div><div><span>amount</span><strong>Positive amount in KES</strong></div><div><span>accountReference</span><strong>Starts with 1, max 64 characters</strong></div><div><span>transactionDesc</span><strong>Optional payment description</strong></div><div><span>CheckoutRequestID</span><strong>Save the returned payment identifier</strong></div></div><p className="settings-note"><ShieldCheck size={15} /> A successful HTTP response means Safaricom accepted the request. Confirm the final payment status from your LeeTec dashboard before fulfilling an order.</p></section>
        <section className="settings-card docs-panel"><div className="settings-card-heading"><div className="settings-icon"><Webhook size={16} /></div><div><h2>Callbacks and production setup</h2><p>LeeTec manages the Safaricom callback endpoints for the shared production connection.</p></div></div><p className="settings-note">Users do not need to expose their own callback URL or configure Daraja credentials. LeeTec receives the Safaricom callback, validates the payment, and updates the transaction record.</p><div className="doc-detail-list"><div><span>Environment</span><strong>Production</strong></div><div><span>STK type</span><strong>CustomerBuyGoodsOnline</strong></div><div><span>Authentication</span><strong>One sk_live_ secret</strong></div></div></section>
