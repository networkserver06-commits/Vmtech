...
function normalizePayoutDestination(value: string) {
  ...
  return normalizeDepositPhone(value);
}
...
export default function Home() {
  ...
  const [walletHistoryTab, setWalletHistoryTab] = useState<"deposits" | "collections" | "charges">("deposits");
  ...
  const [paymentLinkMode, setPaymentLinkMode] = useState<"fixed" | "payer">("payer");
  ...
  const deleteApiKey = trpc.engine.deleteApiKey.useMutation({ onSuccess: () => { apiKeys.refetch(); setRevealedKeyId(null); notify("API key permanently deleted"); }, onError: (error) => notify(mutationError(error), "error") });
  const createTill = trpc.engine.createTill.useMutation({ onSuccess: () => { tills.refetch(); setTillNumber(""); setPayoutPhone(""); setTillName(""); setTillLocation(""); setTillType("BUY_GOODS"); notify("Payment destination saved. The payout phone or Till number and destination details were confirmed."); }, onError: (error) => notify(mutationError(error), "error") });
  ...
  const submitTill = () => {
    ...
    const payoutDestination = normalizePayoutDestination(payoutPhone);
    if (!payoutDestination) return notify("Enter a valid payout phone (07…, 01…, 254…) or a 5–8 digit Till/PayBill number", "error");
    createTill.mutate({ name: tillName.trim(), tillNumber, payoutPhone: payoutDestination, location: tillLocation.trim() || undefined, paymentType: tillType });
  };
  const submitKey = () => {
  ...
  const generatePaymentLink = () => {
    ...
    if (paymentLinkMode === "fixed") url.searchParams.set("amount", amount.toFixed(2));
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap dashboard-tabs" data-active-tab={activeNav}>
          ...
          <section className="panel payment-links-panel tab-payment-links"><div className="panel-heading"><div><div className="panel-kicker">SHAREABLE COLLECTIONS <span className="eyebrow-line" /></div><h3>Generate a payment link</h3><p>Create a secure checkout link with a fixed amount or let each payer enter their own amount.</p></div><Link2 size={18} className="gold-icon" /></div><div className="key-access-note"><ShieldCheck size={15} /><div><strong>Username-based checkout</strong><span>Your link is generated as <code>/pay/username</code>. Amount is optional: choose a fixed amount for a set price, or let the payer enter the amount at checkout.</span></div></div><div className="payment-link-modes"><button type="button" className={paymentLinkMode === "fixed" ? "active" : ""} onClick={() => setPaymentLinkMode("fixed")}><strong>Fixed amount</strong><span>Customer pays the exact amount you set.</span></button><button type="button" className={paymentLinkMode === "payer" ? "active" : ""} onClick={() => { setPaymentLinkMode("payer"); setPaymentLinkAmount(""); }}><strong>Payer enters amount</strong><span>Leave the amount open for flexible requests.</span></button></div><div className="webhook-create-row payment-link-form"><label><span>{paymentLinkMode === "fixed" ? "Amount in KES" : "Amount in KES (optional)"}</span><Input value={paymentLinkAmount} onChange={(event) => setPaymentLinkAmount(event.target.value.replace(/[^\d.]/g, ""))} placeholder={paymentLinkMode === "fixed" ? "e.g. 250" : "Leave blank for payer-entered amount"} aria-label="Payment link amount" inputMode="decimal" /></label><label><span>Reference (optional)</span><Input value={paymentLinkReference} onChange={(event) => setPaymentLinkReference(event.target.value.replace(/[^A-Za-z0-9_-]/g, "").slice(0, 64))} placeholder="e.g. 1ORDER1001" aria-label="Payment link reference" /></label><button className="primary-button" onClick={generatePaymentLink}><Link2 size={16} /> Generate link</button></div>{paymentLink && <div className="generated-link-card"><div><strong>Ready to share</strong><small>{paymentLinkMode === "fixed" ? `Fixed amount · ${money(Number(paymentLinkAmount))}` : "Payer enters amount at checkout"}</small><span>{paymentLink}</span></div><div className="generated-link-actions"><CopyButton value={paymentLink} /><button className="secondary-button" onClick={() => window.open(paymentLink, "_blank", "noopener,noreferrer")}><ExternalLink size={15} /> Open form</button></div></div>}</section>

          <section className="dashboard-grid tab-overview">
            <div className="panel volume-panel"><div className="panel-heading"><div><h3>Transaction volume</h3><p>Gross collection value processed through LeeTec</p></div><div className="period-select">All recorded data <ChevronDown size={14} /></div></div><div className="chart-summary"><div><strong>{money(live.collections)}</strong><span>Collections</span></div><div className="legend"><span><i className="legend-dot collections" /> Collections</span></div></div><div className="bar-chart" aria-label="Transaction volume chart">{chartBars.map((height, index) => <div className="bar-column" key={index}><div className="bar collections" style={{ height: `${height}%` }} /></div>)}</div><div className="chart-axis"><span>Older</span><span>Recent</span></div></div>
            <div className="panel quick-panel"><div className="panel-heading"><div><h3>Quick actions</h3><p>Common developer tasks</p></div><Zap size={17} className="gold-icon" /></div><div className="quick-list"><button onClick={() => navigate("/collections?from=dashboard")}><span className="quick-icon green"><ArrowDownLeft size={17} /></span><span><strong>Collect payment</strong><small>Trigger an STK Push</small></span><ArrowUpRight size={15} /></button><button onClick={() => navigate("/collections?from=dashboard")}><span className="quick-icon blue"><Store size={17} /></span><span><strong>Pay to a Till</strong><small>Send an STK Push to a customer</small></span><ArrowUpRight size={15} /></button></div></div>
          </section>

          <section className="lower-grid">
            <div className="panel activity-panel tab-overview"><div className="panel-heading"><div><h3>Recent activity</h3><p>Your latest collections</p></div><button className="panel-link" onClick={() => navigate("/collections?from=dashboard")}>View all <ArrowUpRight size={14} /></button></div><div className="activity-table"><div className="table-head"><span>Reference</span><span>Type</span><span>Amount</span><span>Status</span><span>Time</span><span /></div>{activities.length ? activities.map((tx) => { const payout = false; const status = String(tx.status ?? "PENDING"); const amount = Number(tx.amount ?? 0); const reference = String(tx.checkoutRequestId ?? tx.conversationId ?? tx.id ?? "—"); const detail = String(tx.phoneNumber ?? tx.recipientPhone ?? tx.accountReference ?? "—"); const displayStatus = status === "SUCCESS" ? "Success" : status === "FAILED" ? "Failed" : "Pending"; return <div className="table-row" key={`${tx.kind}-${tx.id}`}><div className="ref-cell"><div className={`tx-icon ${payout ? "payout" : "collection"}`}>{payout ? <Send size={14} /> : <ArrowDownLeft size={14} />}</div><div><strong>{reference}</strong><small>{detail}</small></div></div><span className="type-cell">STK Push / C2B</span><div className="activity-amount-cell"><strong className={payout ? "amount-negative" : "amount-positive"}>+ {money(amount)}</strong>{!payout && <small>Fee {money(Number(tx.platformFee ?? 0))} · Net {money(Number(tx.netAmount ?? amount))}</small>}</div><StatusBadge status={displayStatus} /><span className="time-cell">{relativeTime(tx.createdAt)}</span><button className="row-more" aria-label={`More actions for ${reference}`}><MoreHorizontal size={16} /></button></div>; }) : <div className="live-empty">No M-PESA transactions yet. New activity will appear here automatically.</div>}</div></div>
            <div id="api-keys-panel" className="panel key-panel tab-api-keys"><div className="panel-heading"><div><h3>Developer API keys</h3><p>Create a secret key for server-side API requests.</p></div><KeyRound size={17} className="gold-icon" /></div><div className="key-access-note"><ShieldCheck size={15} /><div><strong>Use the secret key for live API calls</strong><span>Send it as <code>Authorization: Bearer sk_live_…</code> or the <code>x-api-key</code> header. New keys are encrypted for owner-only recovery.</span></div></div><div className="key-preview"><div className="key-label"><span>{activeKey ? String(activeKey.name) : "No active key"}</span>{activeKey && <button onClick={() => { setRevokeKeyId(Number(activeKey.id)); setKeyConfirmation("revoke"); }}>Revoke</button>}</div><div className="secret-value">{displayedKey ? (showSecret ? displayedKey : "sk_live_••••••••••••••••") : (activeKey ? `${String(activeKey.keyPrefix)}••••••••••••••••` : "Create a key to receive a secret")}{displayedKey && <><button type="button" className="secret-toggle" aria-label={showSecret ? "Hide API key" : "Show API key"} onClick={() => setShowSecret((value) => !value)}>{showSecret ? <EyeOff size={15} /> : <Eye size={15} />}</button><CopyButton value={displayedKey} /></>}</div><div className="key-meta"><span><span className="live-dot" /> {activeKey ? "Active" : "Not configured"}</span><span>{activeKey ? `Created ${relativeTime(activeKey.createdAt)}` : "No keys created"}</span></div></div><div className="key-history-list"><div className="key-history-heading"><strong>All API keys</strong><span>{visibleKeys.length} total</span></div>{visibleKeys.length ? visibleKeys.map((key) => { const keyId = Number(key.id); const keyValue = typeof key.key === "string" ? String(key.key) : null; const isRevealed = revealedKeyId === keyId && Boolean(keyValue); return <div className="key-history-row" key={String(key.id)}><div><strong>{String(key.name ?? "Unnamed key")}</strong><small>{isRevealed ? keyValue : `${String(key.keyPrefix ?? "sk_live_")}••••••••`} · Created {relativeTime(key.createdAt)}</small></div><div className="key-history-actions">{keyValue && <><button type="button" className="secret-toggle" aria-label={isRevealed ? "Hide API key" : "Show API key"} onClick={() => setRevealedKeyId(isRevealed ? null : keyId)}>{isRevealed ? <EyeOff size={14} /> : <Eye size={14} />}</button><CopyButton value={keyValue} /></>}<span className={`till-status ${Boolean(key.isActive) ? "active" : "inactive"}`}>{Boolean(key.isActive) ? "Active" : "Revoked"}</span><button type="button" className="danger-button key-delete-button" onClick={() => { if (window.confirm("Permanently delete this API key? Any integration using it will stop working and this cannot be undone.")) deleteApiKey.mutate({ id: keyId }); }}>Delete</button></div></div>; }) : <div className="till-empty">No API keys have been created.</div>}</div><div className="key-warning"><ShieldCheck size={16} /><span>{activeKey && !recoverableKey && !newSecret ? "This is a legacy key created before recovery was enabled. Create a replacement key to enable eye reveal and copy anytime." : "Encrypted owner-only recovery is enabled. Use the eye to reveal the key and copy it whenever needed."}</span></div><div className="key-create-row"><Input value={keyName} onChange={(event) => setKeyName(event.target.value)} placeholder="Key name, e.g. Production API" aria-label="New API key name" /><button className="secondary-button" disabled={createApiKey.isPending} onClick={submitKey}><Plus size={16} /> Create key</button></div></div>
          </section>
          ...
          <section id="tills-panel" className="panel tills-panel tab-tills">
            ...
            <div className="till-list">{visibleTills.length ? visibleTills.map((till) => <div className="till-row" key={String(till.id)}><div className="till-icon"><Webhook size={15} /></div><div className="till-details"><strong>{String(till.name)}</strong><span>{String(till.paymentType ?? "BUY_GOODS") === "PAYBILL" ? "PayBill" : "Buy Goods Till"} · {String(till.tillNumber)}{till.location ? ` · ${String(till.location)}` : ""}</span><small> Payout destination: {String(till.payoutPhone ?? "Not set")}</small></div><span className={`till-status ${Boolean(till.isActive) ? "active" : "inactive"}`}>{Boolean(till.isActive) ? "Active" : "Inactive"}</span><button className="row-more" aria-label={`Remove ${String(till.name)}`} onClick={() => deleteTill.mutate({ id: Number(till.id) })}><X size={14} /></button></div>) : <div className="till-empty">No payment destinations configured. Add a Buy Goods Till or PayBill to route API collections.</div>}</div>
            <div className="till-create-row"><select value={tillType} onChange={(event) => setTillType(event.target.value as "BUY_GOODS" | "PAYBILL")} aria-label="Payment destination type"><option value="BUY_GOODS">Buy Goods Till</option><option value="PAYBILL">PayBill</option></select><Input value={tillName} onChange={(event) => setTillName(event.target.value)} placeholder="Till name" aria-label="Till name" /><Input value={tillNumber} onChange={(event) => setTillNumber(event.target.value.replace(/\D/g, "").slice(0, 8))} placeholder="Till or PayBill number" aria-label="Till or PayBill number" inputMode="numeric" /><Input value={tillLocation} onChange={(event) => setTillLocation(event.target.value)} placeholder="Location (optional)" aria-label="Location (optional)" /><Input value={payoutPhone} onChange={(event) => setPayoutPhone(event.target.value.replace(/[^\d+\s()-]/g, "").slice(0, 20))} placeholder="Payout phone or Till/PayBill number" aria-label="Payout phone or Till/PayBill number" inputMode="tel" autoComplete="tel" /><button className="secondary-button" disabled={createTill.isPending} onClick={submitTill}><Plus size={16} /> Add destination</button></div>
            <div className="fee-note"><ShieldCheck size={15} /><span>Add either the Kenyan phone number or Till/PayBill number that should receive payouts. Confirm the destination carefully before saving; it is stored for payout review.</span></div>
          </section>
        ...
        {profileOpen && <div className="crud-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setProfileOpen(false); }}><div className="crud-modal profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-title"><div className="modal-heading"><div><span className="modal-kicker">WORKSPACE SETTINGS</span><h2 id="profile-title">Profile & account</h2><p>Keep your workspace identity and session controls up to date.</p></div><button aria-label="Close profile settings" onClick={() => setProfileOpen(false)}><X size={18} /></button></div><div className="profile-summary"><div className="profile-avatar large">{user.name?.slice(0, 2).toUpperCase() ?? "ME"}</div><div><strong>{user.name ?? "Developer"}</strong><span>{user.email ?? "No email"}</span><small>Username: <b>{user.username ?? "—"}</b> · Account #{user.accountId ?? "—"} · {user.role === "admin" ? "Administrator" : "Developer"}</small></div></div><div className="form-grid profile-form"><label>Display name<Input value={profileName} onChange={(event) => setProfileName(event.target.value)} placeholder="Your full name" autoComplete="name" /></label><label>Permanent username<Input value={user.username ?? "—"} readOnly disabled /></label><label>Email address<Input value={user.email ?? ""} readOnly disabled /></label></div><div className="password-settings-section"><div><strong>Change password</strong><span>Verify your current password before setting a new one. This does not change your permanent username.</span></div><div className="form-grid profile-form"><label>Current password<Input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" /></label><label>New password<Input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" /></label><label>Confirm new password<Input type="password" value={confirmNewPassword} onChange={(event) => setConfirmNewPassword(event.target.value)} autoComplete="new-password" /></label></div><button className="secondary-button" disabled={changePassword.isPending || !currentPassword || newPassword.length < 8 || newPassword !== confirmNewPassword} onClick={() => changePassword.mutate({ currentPassword, newPassword })}>{changePassword.isPending ? "Changing password…" : "Change password"}</button></div><div className="modal-note"><ShieldCheck size={16} /><span>Your email and role are managed securely by the account system. Your permanent username, email, account number, and role cannot be changed here. Only your display name and password can be changed.</span></div><div className="modal-actions"><button className="danger-button" onClick={async () => { await logout(); navigate("/"); }}>Sign out</button><span className="modal-actions-spacer" /><button className="secondary-button" onClick={() => setProfileOpen(false)}>Cancel</button><button className="primary-button" disabled={updateProfile.isPending || profileName.trim().length < 2} onClick={() => updateProfile.mutate({ name: profileName.trim() })}>{updateProfile.isPending ? "Saving…" : "Save profile"}</button></div></div></div>}
