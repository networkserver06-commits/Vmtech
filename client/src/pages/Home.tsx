...
export default function Home() {
  ...
  const createApiKey = trpc.engine.createApiKey.useMutation({ onSuccess: (result) => { setNewSecret(result.key); setShowSecret(true); setKeyName(""); apiKeys.refetch(); notify("Production key created. Copy it now; it will not be shown again."); }, onError: (error) => notify(mutationError(error), "error") });
  ...
  const createTill = trpc.engine.createTill.useMutation({ onSuccess: () => { tills.refetch(); setTillNumber(""); setTillName(""); setTillLocation(""); setTillType("BUY_GOODS"); notify("Payment destination added to your workspace"); }, onError: (error) => notify(mutationError(error), "error") });
