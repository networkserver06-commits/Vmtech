              ...
              {group.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { setActiveNav(item.label); setMobileOpen(false); if (item.label === "Collections") navigate("/collections"); if (item.label === "Payouts") navigate("/payouts"); if (item.label === "API keys" || item.label === "Tills") window.setTimeout(() => document.getElementById(item.label === "API keys" ? "api-keys-panel" : "tills-panel")?.scrollIntoView({ behavior: "smooth", block: "center" }), 0); }}
                  >
