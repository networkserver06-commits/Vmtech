...
export default function Home() {
  ...
  const submitTill = () => {
    ...
    const destination = normalizePayoutDestination(tillNumber);
    if (!destination) return notify("Enter either a Kenyan phone number or a 5–8 digit Till/PayBill number", "error");
    createTill.mutate({ name: tillName.trim(), tillNumber: destination, payoutPhone: destination, location: tillLocation.trim() || undefined, paymentType: destination.startsWith("254") ? "PHONE" : tillType });
  };
  const submitKey = () => {
      ...
      <main className="main-content">
        ...
        <div className="content-wrap dashboard-tabs" data-active-tab={activeNav}>
          ...
          <section id="tills-panel" className="panel tills-panel tab-tills">
            <div className="panel-heading"><div><h3>Payment destinations</h3><p>Add one phone number, Buy Goods Till, or PayBill number for payouts and collections.</p></div><span className="till-count">{visibleTills.length} configured</span></div>
            ...
            <div className="fee-note"><ShieldCheck size={15} /><span>Enter either one Kenyan phone number or one Till/PayBill number. The same confirmed destination is stored for payout review and collection routing.</span></div>
          </section>
