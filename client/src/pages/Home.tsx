      ...
      <main className="main-content">
        ...
        <div className="content-wrap dashboard-tabs" data-active-tab={activeNav}>
          ...
          <section className="wallet-deposit-panel panel tab-wallet">
            ...
            {(depositWallet.isPending || depositNotice || depositError) && <div className={`deposit-flow-status ${depositError ? "error" : depositWallet.isPending ? "pending" : "success"}`} role="status"><div className="deposit-flow-icon">{depositError ? <XCircle size={16} /> : depositWallet.isPending ? <Activity size={16} /> : <CheckCircle2 size={16} />}</div><div><strong>{depositError ? "Deposit request failed" : depositWallet.isPending ? "Sending STK prompt…" : "STK prompt request accepted"}</strong><span>{depositError ?? depositNotice}</span>{depositError && <small>Check the live Daraja shortcode, STK passkey, consumer credentials, phone number, and that the M-PESA prompt product is enabled.</small>}</div>{depositError && <button aria-label="Dismiss deposit error" onClick={() => setDepositError(null)}><X size={15} /></button>}</div>}
