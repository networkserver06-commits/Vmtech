...
export default function Home() {
  ...
  const createApiKey = trpc.engine.createApiKey.useMutation({ onSuccess: (result) => { setNewSecret(result.key); setShowSecret(true); setKeyName(""); apiKeys.refetch(); notify("Production key created. Copy it now; it will not be shown again."); } });
  const revokeApiKey = trpc.engine.revokeApiKey.useMutation({ onSuccess: () => { apiKeys.refetch(); notify("API key revoked"); } });
  const createTill = trpc.engine.createTill.useMutation({ onSuccess: () => { tills.refetch(); setTillNumber(""); setTillName(""); setTillLocation(""); setTillType("BUY_GOODS"); notify("Payment destination added to your workspace"); } });
  const deleteTill = trpc.engine.deleteTill.useMutation({ onSuccess: () => { tills.refetch(); notify("Till removed"); } });
  const createWebhook = trpc.engine.createWebhook.useMutation({ onSuccess: () => { webhooks.refetch(); setWebhookUrl(""); setWebhookSecret(""); setShowWebhookSecret(false); notify("Webhook endpoint added"); }, onError: (error) => notify(error.message) });
