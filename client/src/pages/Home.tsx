...
export default function Home() {
  ...
  const createTill = trpc.engine.createTill.useMutation({ onSuccess: () => { tills.refetch(); setTillNumber(""); setPayoutPhone(""); setTillName(""); setTillLocation(""); setTillType("BUY_GOODS"); notify("Payment destination saved. The payout number and Till details were confirmed."); }, onError: (error) => notify(mutationError(error), "error") });
  ...
  const submitTill = () => {
    ...
    const normalizedPayoutPhone = normalizeDepositPhone(payoutPhone);
    if (!normalizedPayoutPhone) return notify("Enter a valid Kenyan phone number for payouts: 07…, 01…, or 254…", "error");
    createTill.mutate({ name: tillName.trim(), tillNumber, payoutPhone: normalizedPayoutPhone, location: tillLocation.trim() || undefined, paymentType: tillType });
  };
  const submitKey = () => {
      ...
      <main className="main-content">
        ...
        <div className="content-wrap dashboard-tabs" data-active-tab={activeNav}>
          ...
          <section id="tills-panel" className="panel tills-panel tab-tills">
            ...
            <div className="till-list">{visibleTills.length ? visibleTills.map((till) => <div className="till-row" key={String(till.id)}><div className="till-icon"><Webhook size={15} /></div><div className="till-details"><strong>{String(till.name)}</strong><span>{String(till.paymentType ?? "BUY_GOODS") === "PAYBILL" ? "PayBill" : "Buy Goods Till"} · {String(till.tillNumber)}{till.location ? ` · ${String(till.location)}` : ""}</span><small> Payout number: {String(till.payoutPhone ?? "Not set")}</small></div><span className={`till-status ${Boolean(till.isActive) ? "active" : "inactive"}`}>{Boolean(till.isActive) ? "Active" : "Inactive"}</span><button className="row-more" aria-label={`Remove ${String(till.name)}`} onClick={() => deleteTill.mutate({ id: Number(till.id) })}><X size={14} /></button></div>) : <div className="till-empty">No payment destinations configured. Add a Buy Goods Till or PayBill to route API collections.</div>}</div>
            <div className="till-create-row"><select value={tillType} onChange={(event) => setTillType(event.target.value as "BUY_GOODS" | "PAYBILL")} aria-label="Payment destination type"><option value="BUY_GOODS">Buy Goods Till</option><option value="PAYBILL">PayBill</option></select><Input value={tillName} onChange={(event) => setTillName(event.target.value)} placeholder="Till name" aria-label="Till name" /><Input value={tillNumber} onChange={(event) => setTillNumber(event.target.value.replace(/\D/g, "").slice(0, 8))} placeholder="Till or PayBill number" aria-label="Till or PayBill number" inputMode="numeric" /><Input value={tillLocation} onChange={(event) => setTillLocation(event.target.value)} placeholder="Location (optional)" aria-label="Location (optional)" /><Input value={payoutPhone} onChange={(event) => setPayoutPhone(event.target.value.replace(/[^\d+\s()-]/g, "").slice(0, 20))} placeholder="Payout phone used for payouts" aria-label="Payout phone used for payouts" inputMode="tel" autoComplete="tel" /><button className="secondary-button" disabled={createTill.isPending} onClick={submitTill}><Plus size={16} /> Add destination</button></div>
            <div className="fee-note"><ShieldCheck size={15} /><span>Add the Kenyan phone number that should receive payouts for this destination. Confirm the Till/PayBill number and payout phone carefully before saving; the details are stored for payout review.</span></div>
          </section>
