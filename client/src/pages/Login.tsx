...
export default function Login() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); if (params.get("mode") === "register") setMode("register"); if (params.get("verified") === "1") setNotice("Email verified. You can now sign in."); if (params.get("verified") === "0") setError("That verification link is invalid or expired."); }, []);
  const submit = async () => {
    setError(""); setNotice("");
