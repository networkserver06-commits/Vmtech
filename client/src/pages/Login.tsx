...
export default function Login() {
  ...
  const showError = (message: string) => { setError(message); setToast(message); window.setTimeout(() => setToast(null), 3200); };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!form.email.trim()) return showError("Enter your email address.");
    if (!form.password) return showError("Enter your password.");
    if (mode === "register" && !form.name.trim()) return showError("Enter your full name.");
    if (mode === "register" && !passwordChecks.length) return showError("Password must be at least 8 characters.");
    if (mode === "register" && !passwordChecks.match) return showError("Passwords do not match.");
    setBusy(true);
    try {
      ...
      if (mode === "register") {
        setNotice(body.verificationSent ? "Account created. Email verification is required. Check your inbox, verify your email, then sign in." : "Account created, but the required verification email could not be sent. Check your email address and resend it below.");
        setMode("login"); setForm({ name: "", email: form.email, password: "", confirmPassword: "" });
      }
      else {
        const requested = new URLSearchParams(window.location.search).get("returnTo");
        navigate(requested && requested.startsWith("/") && !requested.startsWith("//") ? requested : "/dashboard");
      }
    } catch (err) { showError(err instanceof Error ? err.message : "Unable to connect to LeeTec Engine."); }
    finally { setBusy(false); }
  };
  const resend = async () => {
    if (!form.email) return showError("Enter your email address first.");
